...
function CategoryBadge({ category, locale }: { category: TipCategory; locale: Locale }) {
  ...
  return (
    <span className={`text-[11px] font-medium px-2 py-0.5 rounded-full ${colors[category]}`}>
...
function ArticleCard({ tip, locale }: { tip: TipMeta; locale: Locale }) {
  ...
  return (
    <Link
      ...
      className="group flex flex-col rounded-2xl border border-zinc-200 dark:border-dark-border bg-white/70 dark:bg-dark-surface/70 backdrop-blur-sm overflow-hidden hover:border-indigo/30 hover:shadow-lg hover:shadow-indigo/5 hover:-translate-y-0.5 transition-all duration-200"
    >
      {/* Color header bar */}
      <div className={`h-1.5 ${isLeadMagnet ? "bg-gradient-to-r from-electric-yellow to-peach" : "bg-gradient-to-r from-indigo to-indigo/60"}`} />

      <div className="p-5 flex flex-col flex-1">
        <div className="flex items-center gap-2 mb-3">
          <CategoryBadge category={tip.category} locale={locale} />
          {isLeadMagnet && tip.downloadLabel && (
            <span className="text-[11px] font-medium px-2 py-0.5 rounded-full bg-peach/15 text-peach flex items-center gap-1">
        ...
        <div className="mt-4 flex items-center text-sm font-medium text-indigo opacity-0 group-hover:opacity-100 transition-opacity">
          {isLeadMagnet ? ui.downloadPdf : "Lesen"}
...
function FeaturedCard({ tip, locale }: { tip: TipMeta; locale: Locale }) {
  ...
  return (
    <Link
      href={`/${locale}/tips/${tip.slug}`}
      className="group relative flex flex-col sm:flex-row rounded-2xl border border-indigo/20 bg-gradient-to-br from-white to-lavender/40 dark:from-dark-surface dark:to-indigo/5 overflow-hidden hover:shadow-xl hover:shadow-indigo/10 hover:-translate-y-0.5 transition-all duration-200"
    >
      <div className="absolute top-4 left-4">
        <span className="text-[11px] font-bold px-2.5 py-1 rounded-full bg-indigo text-white uppercase tracking-wider">
          {ui.featured}
        </span>
      </div>
      <div className="p-6 sm:p-8 pt-12 flex-1">
        ...
        <h2 className="text-xl sm:text-2xl font-bold text-dark-text dark:text-white group-hover:text-indigo transition-colors mb-3">
          {tip.title}
        </h2>
        <p className="text-sm text-zinc-500 dark:text-zinc-400 max-w-lg">
          {tip.description}
        </p>
        <div className="mt-4 inline-flex items-center gap-1 text-sm font-semibold text-indigo">
          Lesen <span className="group-hover:translate-x-0.5 transition-transform">&gt;&gt;</span>
        </div>
      </div>
      <div className="hidden sm:flex items-center justify-center px-8">
        <KeyCharacter pose="sitting" size={100} />
      </div>
    </Link>
  );
}
...
function LeadMagnetBanner({ locale }: { locale: Locale }) {
  ...
  const bannerText: Record<Locale, { title: string; desc: string }> = {
    de: { title: "Tastenkombinationen als Cheat Sheet", desc: "Die wichtigsten Shortcuts für Windows und Mac - zum Ausdrucken." },
    en: { title: "Keyboard shortcuts cheat sheet", desc: "The most important shortcuts for Windows and Mac - printable." },
    fr: { title: "Raccourcis clavier en aide-mémoire", desc: "Les raccourcis indispensables pour Windows et Mac - à imprimer." },
  };

  return (
    <div className="rounded-2xl border border-electric-yellow/30 bg-gradient-to-r from-electric-yellow/10 to-peach/10 dark:from-electric-yellow/5 dark:to-peach/5 p-6 flex flex-col sm:flex-row items-center gap-4">
...
export function TipsOverview({ tips, locale }: Props) {
  ...
  const ui = tipsUi[locale];
  const categories = Object.keys(categoryLabels[locale]) as (TipCategory | "all")[];
    ...
    <div className="mx-auto max-w-4xl px-6 py-10 space-y-8">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all duration-150 ${
                activeCategory === cat
                  ...
                  : "bg-white/70 dark:bg-dark-surface/70 text-zinc-600 dark:text-zinc-400 hover:bg-indigo/10 hover:text-indigo border border-zinc-200 dark:border-dark-border"
              }`}
            >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {nonFeatured.map((tip, i) => (
          <ScrollReveal key={tip.slug} delay={80 + i * 60}>
      ...
      <ScrollReveal delay={240}>
        <div className="rounded-2xl bg-gradient-to-br from-indigo to-indigo/80 p-8 text-center text-white space-y-4">
          <h2 className="text-xl font-bold">{ui.tryCta}</h2>
          <p className="text-white/80 text-sm max-w-md mx-auto">{ui.tryCtaDesc}</p>
